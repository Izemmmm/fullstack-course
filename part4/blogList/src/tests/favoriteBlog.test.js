...
describe('favorite blog', () => {
  ...
  test('with one blog in the list', () => {
    assert.deepStrictEqual(favoriteBlog(oneBlogList), oneBlogList[0]);
  });

  test('with filled list', () => {
    assert.deepStrictEqual(favoriteBlog(filledList), filledList[3]);
  });

  test('with filled list, with several blogs with the same amount of likes', () => {
    assert.deepStrictEqual(favoriteBlog(filledListSameLikes), filledListSameLikes[1]);
  });
});